#region Local Var
	var uLocal_0 = 7;
	var uLocal_1 = 0;
	var uLocal_2 = 0;
	var uLocal_3 = 0;
	var uLocal_4 = 0;
	var uLocal_5 = 0;
	var uLocal_6 = 0;
	var uLocal_7 = 0;
	var uLocal_8 = -1;
	var uLocal_9 = 0;
	var uLocal_10 = 0;
	var uLocal_11 = 0;
	var uLocal_12 = 0;
	BOOL bLocal_13 = 0;
	Hash hLocal_14 = 0;
	Any anLocal_15 = 0;
	int iLocal_16 = 0;
	var uLocal_17 = 0;
	var uLocal_18 = 0;
	float fLocal_19 = 0f;
	float fLocal_20 = 0f;
	Ped pedLocal_21 = 0;
	var uLocal_22 = 0;
	var uLocal_23 = 10;
	var uLocal_24 = 0;
	var uLocal_25 = 0;
	var uLocal_26 = 0;
	var uLocal_27 = 0;
	var uLocal_28 = 0;
	var uLocal_29 = -1;
	var uLocal_30 = 2;
	var uLocal_31 = 0;
	var uLocal_32 = 0;
	var uLocal_33 = 10;
	var uLocal_34 = 0;
	var uLocal_35 = 0;
	var uLocal_36 = 0;
	var uLocal_37 = 0;
	var uLocal_38 = 0;
	var uLocal_39 = 0;
	var uLocal_40 = 0;
	var uLocal_41 = 0;
	var uLocal_42 = 0;
	var uLocal_43 = 0;
	var uLocal_44 = 2;
	var uLocal_45 = 0;
	var uLocal_46 = 0;
	var uLocal_47 = 10;
	var uLocal_48 = 0;
	var uLocal_49 = 0;
	var uLocal_50 = 0;
	var uLocal_51 = 0;
	var uLocal_52 = 0;
	var uLocal_53 = 0;
	var uLocal_54 = 0;
	var uLocal_55 = 0;
	var uLocal_56 = 0;
	var uLocal_57 = 0;
	var uLocal_58 = 10;
	var uLocal_59 = 0;
	var uLocal_60 = 0;
	var uLocal_61 = 0;
	var uLocal_62 = 0;
	var uLocal_63 = 0;
	var uLocal_64 = 0;
	var uLocal_65 = 0;
	var uLocal_66 = 0;
	var uLocal_67 = 0;
	var uLocal_68 = 0;
	var uLocal_69 = 0;
	var uLocal_70 = 0;
	var uLocal_71 = 0;
	var uLocal_72 = 0;
	var uLocal_73 = 0;
	var uLocal_74 = 0;
	var uLocal_75 = 0;
	var uLocal_76 = 0;
	var uLocal_77 = 0;
	var uLocal_78 = 0;
	var uLocal_79 = 0;
	var uLocal_80 = 0;
	var uLocal_81 = 0;
	var uLocal_82 = 0;
	var uLocal_83 = 0;
	var uLocal_84 = 0;
	var uLocal_85 = 0;
	var uLocal_86 = 0;
	var uLocal_87 = 0;
	var uLocal_88 = 0;
	var uLocal_89 = 10;
	var uLocal_90 = 0;
	var uLocal_91 = 0;
	var uLocal_92 = 0;
	var uLocal_93 = 0;
	var uLocal_94 = 0;
	var uLocal_95 = 0;
	var uLocal_96 = 0;
	var uLocal_97 = 0;
	var uLocal_98 = 0;
	var uLocal_99 = 0;
	var uLocal_100 = 0;
	var uLocal_101 = 0;
	var uLocal_102 = 0;
	var uLocal_103 = 0;
	var uLocal_104 = 0;
	var uLocal_105 = 0;
	var uLocal_106 = 0;
	var uLocal_107 = 0;
	var uLocal_108 = 0;
	var uLocal_109 = 0;
	var uLocal_110 = 0;
	var uLocal_111 = 0;
	var uLocal_112 = 0;
	var uLocal_113 = 0;
	var uLocal_114 = 0;
	var uLocal_115 = 0;
	var uLocal_116 = 0;
	var uLocal_117 = 0;
	var uLocal_118 = 0;
	var uLocal_119 = 0;
	var uLocal_120 = 3;
	var uLocal_121 = 0;
	var uLocal_122 = 0;
	var uLocal_123 = 0;
	var uLocal_124 = 10;
	var uLocal_125 = 0;
	var uLocal_126 = 0;
	var uLocal_127 = 0;
	var uLocal_128 = 0;
	var uLocal_129 = 0;
	var uLocal_130 = 0;
	var uLocal_131 = 0;
	var uLocal_132 = 0;
	var uLocal_133 = 0;
	var uLocal_134 = 0;
	var uLocal_135 = 0;
	var uLocal_136 = 0;
	var uLocal_137 = 0;
	var uLocal_138 = 0;
	var uLocal_139 = 0;
	var uLocal_140 = 0;
	var uLocal_141 = 0;
	var uLocal_142 = 0;
	var uLocal_143 = 0;
	var uLocal_144 = 0;
	var uLocal_145 = 0;
	var uLocal_146 = 0;
	var uLocal_147 = 0;
	var uLocal_148 = 0;
	var uLocal_149 = 0;
	var uLocal_150 = 0;
	var uLocal_151 = 0;
	var uLocal_152 = 0;
	var uLocal_153 = 0;
	var uLocal_154 = 0;
	var uLocal_155 = 0;
	var uLocal_156 = 0;
	var uLocal_157 = 0;
	var uLocal_158 = 0;
	var uLocal_159 = 0;
	var uLocal_160 = 0;
	var uLocal_161 = 0;
	var uLocal_162 = 0;
	var uLocal_163 = 0;
	var uLocal_164 = 0;
	var uLocal_165 = 0;
	var uLocal_166 = 0;
	var uLocal_167 = 0;
	var uLocal_168 = 0;
	var uLocal_169 = 0;
	var uLocal_170 = 0;
	var uLocal_171 = 0;
	var uLocal_172 = 0;
	var uLocal_173 = 0;
	var uLocal_174 = 0;
	var uLocal_175 = 10;
	var uLocal_176 = 0;
	var uLocal_177 = 0;
	var uLocal_178 = 0;
	var uLocal_179 = 0;
	var uLocal_180 = 0;
	var uLocal_181 = 0;
	var uLocal_182 = 0;
	var uLocal_183 = 0;
	var uLocal_184 = 0;
	var uLocal_185 = 0;
	int iLocal_186 = 0;
	var uLocal_187 = 10;
	var uLocal_188 = 0;
	var uLocal_189 = 0;
	var uLocal_190 = 0;
	var uLocal_191 = 0;
	var uLocal_192 = 0;
	var uLocal_193 = 0;
	var uLocal_194 = 0;
	var uLocal_195 = 0;
	var uLocal_196 = 0;
	var uLocal_197 = 0;
	int iLocal_198 = 0;
	var uLocal_199 = 10;
	var uLocal_200 = 0;
	var uLocal_201 = 0;
	var uLocal_202 = 0;
	var uLocal_203 = 0;
	var uLocal_204 = 0;
	var uLocal_205 = 0;
	var uLocal_206 = 0;
	var uLocal_207 = 0;
	var uLocal_208 = 0;
	var uLocal_209 = 0;
	Object obLocal_210 = 0;
	int iLocal_211 = 0;
	var uLocal_212 = 0;
	var uLocal_213 = 0;
	var uLocal_214 = 0;
	int iLocal_215 = 0;
	BOOL bLocal_216 = 0;
	BOOL bLocal_217 = 0;
	BOOL bLocal_218 = 0;
	int iLocal_219 = 0;
	Any anScriptParam_0 = 0;
	var uScriptParam_1 = 0;
	var uScriptParam_2 = 0;
#endregion

void main() // Position - 0x0 Signature - 22 00 02 00 00 09
{
	bLocal_13 = 1;
	fLocal_19 = 1f;
	fLocal_20 = 1f;
	anLocal_15 = anScriptParam_0;
	iLocal_16 = anScriptParam_0.f_1;
	uLocal_17 = anScriptParam_0.f_2;
	func_1();

	if (PLAYER::HAS_FORCE_CLEANUP_OCCURRED(523))
		func_2();

	while (!SCRIPTS::IS_THREAD_EXIT_REQUESTED() && func_3())
	{
		func_4();
		func_5();
		func_6();
		BUILTIN::WAIT(0);
	}

	func_2();
	return;
}

void func_1() // Position - 0x5C Signature - 22 00 02 00 00 50 00 00 22 00 02
{
	return;
}

void func_2() // Position - 0x64 Signature - 22 00 02 00 00 39
{
	func_7();
	SCRIPTS::_0xE7282390542F570D(anLocal_15);

	if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS())
		func_8();
	else
		SCRIPTS::TERMINATE_THIS_THREAD();

	return;
}

BOOL func_3() // Position - 0x89 Signature - 22 00 02 00 00 54 0E
{
	if (hLocal_14 != 0)
		if (SCRIPTS::GET_NUMBER_OF_THREADS_RUNNING_THE_SCRIPT_WITH_THIS_HASH(hLocal_14) == 0)
			return bLocal_13;
		else
			return true;

	return bLocal_13;
}

void func_4() // Position - 0xB2 Signature - 22 00 02 00 00 50 00 00 22 00 03
{
	return;
}

void func_5() // Position - 0xBA Signature - 22 00 03 00 00 2F 67 02 66 02 11
{
	int i;

	for (i = 0; i < SCRIPTS::GET_NUMBER_OF_EVENTS(SCRIPT_EVENT_QUEUE_ERRORS); i = i + 1)
	{
		switch (SCRIPTS::GET_EVENT_AT_INDEX(SCRIPT_EVENT_QUEUE_ERRORS, i))
		{
			case -843555838:
				func_11(i);
				break;
		
			case -456923784:
				func_10(i);
				break;
		
			case -45008988:
				func_9(i);
				break;
		}
	}

	return;
}

void func_6() // Position - 0x112 Signature - 22 00 10
{
	var buffer;
	var entityCoords;
	var entityRotation;
	var buffer2;

	if (!ENTITY::DOES_ENTITY_EXIST(pedLocal_21))
		pedLocal_21 = PLAYER::PLAYER_PED_ID();

	if (!ENTITY::DOES_ENTITY_EXIST(pedLocal_21.f_5))
	{
		func_12(&pedLocal_21);
		bLocal_216 = true;
		pedLocal_21.f_6 = iLocal_16;
		pedLocal_21.f_7 = uLocal_17;
		pedLocal_21.f_5 = TASK::_GET_SCENARIO_POINT_ENTITY(iLocal_16);
		func_13();
	
		if (ENTITY::DOES_ENTITY_EXIST(pedLocal_21.f_5))
		{
			obLocal_210 = ENTITY::GET_OBJECT_INDEX_FROM_ENTITY_INDEX(pedLocal_21.f_5);
		
			if (!func_14(iLocal_186, 4))
				if (func_15(&pedLocal_21))
					if (!NETWORK::NETWORK_GET_ENTITY_IS_NETWORKED(pedLocal_21.f_5))
						NETWORK::NETWORK_REGISTER_ENTITY_AS_NETWORKED(pedLocal_21.f_5);
				else
					func_16(&iLocal_186, 4);
		
			OBJECT::CREATE_OBJECT_SKELETON(ENTITY::GET_OBJECT_INDEX_FROM_ENTITY_INDEX(pedLocal_21.f_5));
		}
	}
	else if (!func_14(iLocal_186, 1))
	{
		if (!func_14(iLocal_186, 4))
		{
			if (NETWORK::NETWORK_HAS_CONTROL_OF_ENTITY(pedLocal_21.f_5))
			{
				func_16(&iLocal_186, 1);
			}
			else if (!func_14(iLocal_186, 2))
			{
				NETWORK::NETWORK_REQUEST_CONTROL_OF_ENTITY(pedLocal_21.f_5);
				func_16(&iLocal_186, 2);
			}
		}
		else
		{
			func_16(&iLocal_186, 2);
			func_16(&iLocal_186, 1);
		}
	}

	switch (iLocal_215)
	{
		case 0:
			if (!ENTITY::DOES_ENTITY_EXIST(pedLocal_21.f_5))
			{
			}
			else if (!func_14(iLocal_186, 1))
			{
			}
			else if (!OBJECT::CREATE_OBJECT_SKELETON(ENTITY::GET_OBJECT_INDEX_FROM_ENTITY_INDEX(pedLocal_21.f_5)))
			{
			}
			else
			{
				if (!TASK::DOES_SCENARIO_POINT_EXIST(pedLocal_21.f_6))
				{
					bLocal_13 = false;
					return;
				}
			
				if (TASK::_GET_SCENARIO_CONTAINER_OPENING_STATE(pedLocal_21.f_5))
				{
					bLocal_13 = false;
					return;
				}
			
				func_17(&pedLocal_21);
				func_18(&pedLocal_21);
				bLocal_218 = func_19(&pedLocal_21);
			
				if (bLocal_218)
				{
					pedLocal_21.f_3 = !func_20(pedLocal_21.f_5);
				}
				else
				{
					pedLocal_21.f_3 = 1;
					func_21(0, 0f, 0f, 0f, &obLocal_210, 0, 0);
				}
			
				iLocal_211 = func_22(&pedLocal_21, bLocal_218);
				func_23(pedLocal_21.f_5);
				pedLocal_21.f_1 = ENTITY::GET_ENTITY_MODEL(pedLocal_21.f_5);
				func_24(&pedLocal_21);
				func_25(&uLocal_212);
				iLocal_215 = 1;
			}
			break;
	
		case 1:
			if (!TASK::DOES_SCENARIO_POINT_EXIST(pedLocal_21.f_6))
			{
				bLocal_13 = false;
				return;
			}
		
			if (bLocal_216)
			{
				for (iLocal_219 = 0; iLocal_219 < 10; iLocal_219 = iLocal_219 + 1)
				{
					MISC::_INT_TO_STRING(iLocal_219, "%i", &buffer);
					func_27(&uLocal_187[iLocal_219], &pedLocal_21, iLocal_219, &uLocal_199[iLocal_219], &buffer, false, true, true, func_26(&uLocal_212, 2f));
				}
			
				if (ENTITY::DOES_ENTITY_EXIST(pedLocal_21.f_5))
				{
					entityCoords = { ENTITY::GET_ENTITY_COORDS(pedLocal_21.f_5, true, false) };
					entityRotation = { ENTITY::GET_ENTITY_ROTATION(pedLocal_21.f_5, 2) };
				}
				else
				{
					entityCoords = { TASK::_GET_SCENARIO_POINT_COORDS(iLocal_16, true) };
					entityRotation = { 0f, 0f, TASK::_GET_SCENARIO_POINT_HEADING(iLocal_16, true) };
				}
			
				func_28(&iLocal_198, &iLocal_211, &pedLocal_21, entityCoords, entityRotation);
			}
		
			bLocal_217 = true;
		
			for (iLocal_219 = 0; iLocal_219 < 10; iLocal_219 = iLocal_219 + 1)
			{
				if (uLocal_187[iLocal_219] < 2)
				{
					bLocal_217 = false;
					break;
				}
			}
		
			if (iLocal_198 != 4)
				bLocal_217 = false;
		
			if (bLocal_217)
			{
				func_25(&uLocal_212);
				iLocal_215 = 2;
			}
			break;
	
		case 2:
			if (!TASK::DOES_SCENARIO_POINT_EXIST(pedLocal_21.f_6))
			{
				bLocal_13 = false;
				return;
			}
		
			if (bLocal_216)
			{
				for (iLocal_219 = 0; iLocal_219 < 10; iLocal_219 = iLocal_219 + 1)
				{
					MISC::_INT_TO_STRING(iLocal_219, "%i", &buffer2);
					func_27(&uLocal_187[iLocal_219], &pedLocal_21, iLocal_219, &uLocal_199[iLocal_219], &buffer2, false, true, false, func_26(&uLocal_212, 2f));
				}
			}
		
			bLocal_217 = true;
		
			for (iLocal_219 = 0; iLocal_219 < 10; iLocal_219 = iLocal_219 + 1)
			{
				if (uLocal_187[iLocal_219] != 4)
				{
					bLocal_217 = false;
					break;
				}
			}
		
			if (bLocal_217)
			{
				func_29(&uLocal_212);
				func_30(&pedLocal_21);
				iLocal_215 = 3;
			}
			break;
	
		case 3:
			if (!TASK::DOES_SCENARIO_POINT_EXIST(pedLocal_21.f_6) || !PED::IS_PED_USING_THIS_SCENARIO(Global_33, pedLocal_21.f_6))
			{
				bLocal_13 = false;
				return;
			}
			break;
	}

	return;
}

void func_7() // Position - 0x483 Signature - 22 00 02 00 00 89
{
	if (ENTITY::DOES_ENTITY_EXIST(pedLocal_21.f_5))
		if (TASK::_GET_SCENARIO_CONTAINER_OPENING_STATE(pedLocal_21.f_5))
			func_31(pedLocal_21.f_5);

	return;
}

void func_8() // Position - 0x4A9 Signature - 22 00 02 00 00 03
{
	SCRIPTS::TERMINATE_THIS_THREAD();
	return;
}

void func_9(int iParam0) // Position - 0x4B5 Signature - 22 01 05 00 00 11 66 00 4B 03 11 03 11 00 05 41 66 03 54 0F 15 0E 00 11 66 00 09 03 0C 00 35 4B 03 39 F2
{
	struct<2> eventData;

	SCRIPTS::GET_EVENT_DATA(SCRIPT_EVENT_QUEUE_ERRORS, iParam0, &eventData, 2);

	if (eventData == anLocal_15)
	{
		SCRIPTS::SET_EVENT_FLAG_FOR_DELETION(2, iParam0, true);
		func_32(&eventData);
	}

	return;
}

void func_10(int iParam0) // Position - 0x4DD Signature - 22 01 05 00 00 11 66 00 4B 03 11 03 11 00 05 41 66 03 54 0F 15 0E 00 11 66 00 09 03 0C 00 35 4B 03 39 FA
{
	struct<2> eventData;

	SCRIPTS::GET_EVENT_DATA(SCRIPT_EVENT_QUEUE_ERRORS, iParam0, &eventData, 2);

	if (eventData == anLocal_15)
	{
		SCRIPTS::SET_EVENT_FLAG_FOR_DELETION(2, iParam0, true);
		func_33(&eventData);
	}

	return;
}

void func_11(int iParam0) // Position - 0x505 Signature - 22 01 05 00 00 11 66 00 4B 03 11 03 11 00 05 41 66 03 54 0F 15 0E 00 11 66 00 09 03 0C 00 35 4B 03 39 30
{
	struct<2> eventData;

	SCRIPTS::GET_EVENT_DATA(SCRIPT_EVENT_QUEUE_ERRORS, iParam0, &eventData, 2);

	if (eventData == anLocal_15)
	{
		SCRIPTS::SET_EVENT_FLAG_FOR_DELETION(2, iParam0, true);
		func_34(&eventData);
	}

	return;
}

void func_12(var uParam0) // Position - 0x52D Signature - 22 01 04 00 00 2F 66
{
	int i;

	uParam0->f_1 = 0;
	uParam0->f_7 = 0;
	uParam0->f_8 = -1;

	for (i = 0; i < 3; i = i + 1)
	{
		uParam0->f_99[i] = -1;
	}

	return;
}

void func_13() // Position - 0x55F Signature - 22 00 02 00 00 54 BA
{
	if (!func_14(iLocal_186, 8))
	{
		if (!ENTITY::DOES_ENTITY_EXIST(pedLocal_21.f_5))
			return;
	
		if (!func_35(&(pedLocal_21.f_5)))
			func_16(&iLocal_186, 4);
	
		func_16(&iLocal_186, 8);
	}

	return;
}

BOOL func_14(int iParam0, int iParam1) // Position - 0x59D Signature - 22 02 04 00 00 66 00 66 01 69 2F 1C 50 02 01 22 01 03 00 00 66 00 27
{
	return iParam0 && iParam1 != false;
}

BOOL func_15(var uParam0) // Position - 0x5AC Signature - 22 01 03 00 00 66 00 27 05 03
{
	if (!NETWORK::_0x950ACD8F05B7B9DF(uParam0->f_5))
		return false;

	switch (TASK::_GET_SCENARIO_POINT_TYPE(uParam0->f_6))
	{
		case joaat("MP0_DLCSMUGCHARPSTAT_INT235") /* collision: RANSACK_ATTACHED_LOCKBOX_MED_LOCKED_PULLOUT_0m0_0m80 */:
		case joaat("RANSACK_ATTACHED_LOCKBOX_MED_LOCKED_PULLOUT_0m0_1m0"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_LRG_LOCKED_PULLOUT_TIGHT_0m0_0m85"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_MED_LOCKED_PULLOUT_TIGHT_0m0_0m85"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_LRG_LOCKED_PULLOUT_TIGHT_0m0_1m0"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_SML_LOCKED_PULLOUT_0m0_1m0"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_MED_UNLOCKED_PULLOUT_0m0_0m80"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_MED_LOCKED_PULLOUT_0m0_0m80_MP_SYNC_GOTO"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_SML_LOCKED_PULLOUT_TIGHT_0m0_0m70_MP_SYNC_GOTO"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_LRG_LOCKED_PULLOUT_TIGHT_0m0_0m80_MP_SYNC_GOTO"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_SML_LOCKED_PULLOUT_TIGHT_0m0_1m0"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_MED_UNLOCKED_PULLOUT_0m0_0m80_MP_SYNC_GOTO"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_MED_LOCKED_PULLOUT_TIGHT_0m0_0m80"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_LRG_LOCKED_PULLOUT_0m0_0m80_MP_SYNC_GOTO"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_LRG_UNLOCKED_PULLOUT_0m0_0m80_MP_SYNC_GOTO"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_MED_LOCKED_PULLOUT_0m0_0m85"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_LRG_UNLOCKED_PULLOUT_0m0_0m80"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_SML_LOCKED_PULLOUT_TIGHT_0m0_0m70"):
		case joaat("RANSACK_ATTACHED_CHEST_MED_PULLOUT_0m0"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_LRG_LOCKED_PULLOUT_TIGHT_0m0_0m80"):
		case joaat("RANSACK_ATTACHED_CHEST_MED_PULLOUT_0m0_MP_SYNC_GOTO"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_SML_DISCO_BURNED_SETTLEMENT_PULLOUT_0m0_0m70"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_MED_LOCKED_PULLOUT_TIGHT_0m0_0m80_MP_SYNC_GOTO"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_SML_UNLOCKED_PULLOUT_0m0_0m70"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_SML_LOCKED_PULLOUT_0m0_0m70"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_LRG_LOCKED_PULLOUT_0m0_0m80"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_SML_LOCKED_PULLOUT_0m0_0m85"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_SML_UNLOCKED_PULLOUT_0m0_0m70_MP_SYNC_GOTO"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_MED_LOCKED_PULLOUT_TIGHT_0m0_1m0"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_LRG_LOCKED_PULLOUT_0m0_0m85"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_SML_LOCKED_PULLOUT_TIGHT_0m0_0m85"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_SML_LOCKED_PULLOUT_0m0_0m70_MP_SYNC_GOTO"):
		case joaat("RANSACK_ATTACHED_LOCKBOX_LRG_LOCKED_PULLOUT_0m0_1m0"):
			return false;
	}

	return true;
}

void func_16(var uParam0, int iParam1) // Position - 0x6A0 Signature - 22 02 04 00 00 66 00 76 66 01 30 66 00 32 50 02 00 22 01
{
	*uParam0 = *uParam0 || iParam1;
	return;
}

void func_17(var uParam0) // Position - 0x6B1 Signature - 22 01 04 00 00 2F 67 03 66 03 6D
{
	int i;

	for (i = 0; i < 10; i = i + 1)
	{
		uParam0->f_103[i /*5*/] = -1;
		uParam0->f_103[i /*5*/].f_3 = 0;
		uParam0->f_103[i /*5*/].f_1 = 0;
		uParam0->f_103[i /*5*/].f_2 = 0f;
		uParam0->f_103[i /*5*/].f_4 = 0;
		uParam0->f_154[i] = -1;
	}

	return;
}

void func_18(var uParam0) // Position - 0x70A Signature - 22 01 03 00 00 66 00 39 5C
{
	func_36(uParam0);
	func_37(uParam0);
	func_38(uParam0);
	return;
}

BOOL func_19(var uParam0) // Position - 0x724 Signature - 22 01 03 00 00 66 00 27 05 1D
{
	if (DECORATOR::DECOR_EXIST_ON(uParam0->f_5, "loot_allow_random") && DECORATOR::DECOR_GET_BOOL(uParam0->f_5, "loot_allow_random"))
		return 1;

	if (func_39(uParam0))
		return 1;

	if (TASK::_IS_SCENARIO_POINT_FLAG_SET(uParam0->f_6, 25))
		return 0;

	if (func_40(TASK::_GET_SCENARIO_POINT_COORDS(uParam0->f_6, true)))
		return 0;

	return 1;
}

BOOL func_20(Entity eParam0) // Position - 0x77F Signature - 22 01 03 00 00 66 00 6D
{
	if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_empty") || DECORATOR::DECOR_EXIST_ON(eParam0, "loot_money") || DECORATOR::DECOR_EXIST_ON(eParam0, "fLoot_money") || DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_type1") || DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_type2") || DECORATOR::DECOR_EXIST_ON(eParam0, "loot_ammo1_type") || DECORATOR::DECOR_EXIST_ON(eParam0, "loot_ammo2_type") || DECORATOR::DECOR_EXIST_ON(eParam0, "loot_ammo3_type") || DECORATOR::DECOR_EXIST_ON(eParam0, "loot_weapon") || DECORATOR::DECOR_EXIST_ON(eParam0, "loot_capitale"))
		return false;

	return true;
}

void func_21(Hash hParam0, float fParam1, float fParam2, float fParam3, var uParam4, int iParam5, Hash hParam6) // Position - 0x81F Signature - 22 07 09 00 00 66 04
{
	if (!ENTITY::DOES_ENTITY_EXIST(*uParam4))
		*uParam4 = func_41(hParam0, fParam1, iParam5, hParam6);

	if (ENTITY::DOES_ENTITY_EXIST(*uParam4))
		func_42(*uParam4);

	return;
}

int func_22(var uParam0, BOOL bParam1) // Position - 0x854 Signature - 22 02 05 00 00 66 00 27
{
	int _int;

	if (ENTITY::DOES_ENTITY_EXIST(uParam0->f_5) && DECORATOR::DECOR_EXIST_ON(uParam0->f_5, "lootable_interior"))
	{
		_int = DECORATOR::DECOR_GET_INT(uParam0->f_5, "lootable_interior");
	
		if (_int == 0)
			bParam1 = true;
		else
			return _int;
	}

	if (bParam1)
	{
		switch (uParam0->f_8)
		{
			case 0:
				return func_43();
		
			case 1:
				return func_44();
		
			case 16:
				return func_45();
		
			case 17:
				return func_46(uParam0);
		
			default:
			
		}
	}

	return 0;
}

void func_23(Entity eParam0) // Position - 0x8DB Signature - 22 01 0E
{
	int num;
	int value;

	num = OBJECT::_0x250EBB11E81A10BE(eParam0);

	if (num == 0)
		return;

	if (func_14(OBJECT::_0x2BF1953C0C21AC88(num), 1))
	{
		DECORATOR::DECOR_SET_BOOL(eParam0, "loot_empty", true);
		return;
	}

	OBJECT::_0x7D4411D6736CD295(num, &value);

	if (value > 0)
		DECORATOR::DECOR_SET_INT(eParam0, "loot_money", value);

	if (value.f_2 > 0)
	{
		DECORATOR::DECOR_SET_INT(eParam0, "loot_ammo1_type", value.f_1);
		DECORATOR::DECOR_SET_INT(eParam0, "loot_ammo1_amt", value.f_2);
	}

	if (value.f_4 > 0)
	{
		DECORATOR::DECOR_SET_INT(eParam0, "loot_ammo2_type", value.f_3);
		DECORATOR::DECOR_SET_INT(eParam0, "loot_ammo2_amt", value.f_4);
	}

	if (value.f_6 > 0)
	{
		DECORATOR::DECOR_SET_INT(eParam0, "loot_ammo3_type", value.f_5);
		DECORATOR::DECOR_SET_INT(eParam0, "loot_ammo3_amt", value.f_6);
	}

	if (value.f_8 > 0)
	{
		DECORATOR::DECOR_SET_INT(eParam0, "loot_item_type1", value.f_7);
		DECORATOR::DECOR_SET_INT(eParam0, "loot_item_amt1", value.f_8);
	}

	if (value.f_9 != 0)
		DECORATOR::DECOR_SET_INT(eParam0, "loot_weapon", value.f_9);

	return;
}

void func_24(var uParam0) // Position - 0x9D1 Signature - 22 01 1D
{
	int num;
	int i;
	Hash hash;
	int amount;

	if (uParam0->f_3 && func_47(uParam0->f_5))
		return;

	num.f_1 = 10;
	num.f_12 = 10;

	if (!func_48(uParam0->f_5))
		MISC::_LOOT_TABLES_GET_INFO(*uParam0, true, true, uParam0->f_1, &num, 0);

	if (num <= 0 && !uParam0->f_3)
		return;

	if (uParam0->f_3)
		func_49(uParam0, &num, 10);

	if (uParam0->f_2 > 10)
		uParam0->f_2 = 10;

	for (i = 0; i < uParam0->f_2; i = i + 1)
	{
		uParam0->f_26[i] = num.f_1[i];
	
		if (uParam0->f_103[i /*5*/].f_4 != 0)
		{
			uParam0->f_12[i] = uParam0->f_103[i /*5*/].f_4;
			amount = amount + 1;
		}
		else
		{
			hash = TASK::_FIND_MODEL_FOR_ITEM(num.f_1[i]);
		
			if (hash == 0 && uParam0->f_26[i] != 0)
			{
				uParam0->f_12[i] = joaat("scriptedball");
			}
			else
			{
				uParam0->f_12[i] = hash;
				amount = amount + 1;
			}
		}
	}

	if (amount > 0)
		NETWORK::RESERVE_NETWORK_CLIENT_MISSION_OBJECTS(amount);

	return;
}

void func_25(var uParam0) // Position - 0xAEF Signature - 22 01 03 00 00 66 00 73
{
	func_50(uParam0, 0f);
	return;
}

BOOL func_26(var uParam0, float fParam1) // Position - 0xAFE Signature - 22 02 04 00 00 66 00 39
{
	if (!func_51(uParam0))
		return false;

	if (func_52(uParam0) > fParam1)
		return true;

	return false;
}

void func_27(var uParam0, var uParam1, int iParam2, var uParam3, const char* sParam4, BOOL bParam5, BOOL bParam6, BOOL bParam7, BOOL bParam8) // Position - 0xB25 Signature - 22 09 13
{
	Vector3 offsetFromCoordAndHeadingInWorldCoords;
	Vector3 worldPositionOfEntityBone;
	int boneIndex;
	int num;

	switch (*uParam0)
	{
		case 0:
			if (uParam1->f_12[iParam2] != 0)
			{
				STREAMING::REQUEST_MODEL(uParam1->f_12[iParam2], false);
				*uParam0 = 1;
			}
			else
			{
				*uParam0 = 4;
			}
			break;
	
		case 1:
			if (STREAMING::HAS_MODEL_LOADED(uParam1->f_12[iParam2]))
			{
				if (ENTITY::DOES_ENTITY_EXIST(uParam1->f_5))
				{
					if (uParam1->f_154[iParam2] != -1)
					{
						worldPositionOfEntityBone = { ENTITY::GET_WORLD_POSITION_OF_ENTITY_BONE(uParam1->f_5, uParam1->f_154[iParam2]) };
						offsetFromCoordAndHeadingInWorldCoords = { OBJECT::GET_OFFSET_FROM_COORD_AND_HEADING_IN_WORLD_COORDS(worldPositionOfEntityBone, ENTITY::GET_ENTITY_HEADING(uParam1->f_5), uParam1->f_37[iParam2 /*3*/]) };
					}
					else
					{
						offsetFromCoordAndHeadingInWorldCoords = { ENTITY::GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(uParam1->f_5, uParam1->f_37[iParam2 /*3*/]) };
					}
				}
				else if (TASK::DOES_SCENARIO_POINT_EXIST(uParam1->f_6))
				{
					offsetFromCoordAndHeadingInWorldCoords = { OBJECT::GET_OFFSET_FROM_COORD_AND_HEADING_IN_WORLD_COORDS(TASK::_GET_SCENARIO_POINT_COORDS(uParam1->f_6, true), TASK::_GET_SCENARIO_POINT_HEADING(uParam1->f_6, true), uParam1->f_37[iParam2 /*3*/]) };
				}
				else
				{
					break;
				}
			
				*uParam3 = OBJECT::CREATE_OBJECT(uParam1->f_12[iParam2], offsetFromCoordAndHeadingInWorldCoords, func_35(&(uParam1->f_5)), true, false, false, bParam5);
				func_53(uParam1, uParam3, uParam1->f_12[iParam2], uParam1->f_26[iParam2]);
				ENTITY::SET_ENTITY_INVINCIBLE(*uParam3, true);
				OBJECT::_MAKE_ITEM_CARRIABLE(*uParam3);
			
				if (bParam6)
				{
					boneIndex = uParam1->f_4 ? uParam1->f_154[iParam2] : 0;
					offsetFromCoordAndHeadingInWorldCoords = { func_55(uParam1->f_4, uParam1->f_37[iParam2 /*3*/], ENTITY::GET_OFFSET_FROM_ENTITY_GIVEN_WORLD_COORDS(uParam1->f_5, offsetFromCoordAndHeadingInWorldCoords)) };
					ENTITY::ATTACH_ENTITY_TO_ENTITY(*uParam3, uParam1->f_5, boneIndex, offsetFromCoordAndHeadingInWorldCoords, uParam1->f_68[iParam2 /*3*/], false, false, false, false, 2, true, false, false);
				}
				else
				{
					ENTITY::SET_ENTITY_ROTATION(*uParam3, uParam1->f_68[iParam2 /*3*/], 2, true);
				}
			
				*uParam0 = 2;
			}
			else if (bParam8)
			{
			}
			break;
	
		case 2:
			if (!bParam7)
			{
				if (uParam1->f_26[iParam2] != 0)
					num = 3;
				else
					num = 4;
			
				if (MISC::IS_STRING_NULL_OR_EMPTY(sParam4))
					*uParam0 = num;
				else if (!ENTITY::DOES_ENTITY_EXIST(*uParam3))
					*uParam0 = num;
				else if (!TASK::DOES_SCENARIO_POINT_EXIST(uParam1->f_6))
					*uParam0 = num;
				else if (TASK::_ASSOCIATE_PROP_WITH_SCENARIO(uParam1->f_6, *uParam3, sParam4, true))
					*uParam0 = num;
				else if (bParam8)
					*uParam0 = num;
			}
			break;
	
		case 3:
			if (uParam1->f_26[iParam2] != 0)
			{
				TASK::_0xA6A76D666A281F2D(*uParam3, uParam1->f_26[iParam2]);
				*uParam0 = 4;
			}
			break;
	
		case 4:
			break;
	}

	return;
}

void func_28(var uParam0, var uParam1, var uParam2, Vector3 vParam3, var uParam4, var uParam5, var uParam6, var uParam7, var uParam8) // Position - 0xD8E Signature - 22 09 0D
{
	Object entity1;
	int boneIndex;

	switch (*uParam0)
	{
		case 0:
			if (*uParam1 != 0)
			{
				STREAMING::REQUEST_MODEL(*uParam1, false);
				*uParam0 = 1;
			}
			else
			{
				*uParam0 = 4;
			}
			break;
	
		case 1:
			if (STREAMING::HAS_MODEL_LOADED(*uParam1))
			{
				if (*uParam1 == joaat("p_dressinglcmhay01x"))
					vParam3 = { OBJECT::GET_OFFSET_FROM_COORD_AND_HEADING_IN_WORLD_COORDS(vParam3, uParam6.f_2, 0f, 0f, 0.08f) };
			
				entity1 = OBJECT::CREATE_OBJECT(*uParam1, vParam3, func_35(&(uParam2->f_5)), true, false, false, false);
			
				if (ENTITY::DOES_ENTITY_EXIST(uParam2->f_5))
				{
					boneIndex = uParam2->f_4 ? uParam2->f_154[0] : 0;
					ENTITY::ATTACH_ENTITY_TO_ENTITY(entity1, uParam2->f_5, boneIndex, 0f, 0f, 0f, 0f, 0f, 0f, false, false, false, false, 2, true, true, false);
				}
				else
				{
					ENTITY::_SET_ENTITY_COORDS_AND_HEADING_NO_OFFSET(entity1, vParam3, uParam6.f_2, false, true);
				}
			
				*uParam0 = 4;
			}
			break;
	
		case 4:
			break;
	}

	return;
}

void func_29(var uParam0) // Position - 0xE67 Signature - 22 01 03 00 00 73
{
	uParam0->f_1 = 0f;
	uParam0->f_2 = 0f;
	*uParam0 = 0;
	return;
}

void func_30(var uParam0) // Position - 0xE7D Signature - 22 01 03 00 00 66 00 27 05 6D
{
	if (DECORATOR::DECOR_EXIST_ON(uParam0->f_5, "loot_mp_mission_only") && DECORATOR::DECOR_GET_BOOL(uParam0->f_5, "loot_mp_mission_only"))
		DECORATOR::DECOR_REMOVE(uParam0->f_5, "loot_mp_mission_only");

	return;
}

void func_31(Entity eParam0) // Position - 0xEAF Signature - 22 01 03 00 00 66 00 03 05 00 54 8B 0C
{
	if (ENTITY::DOES_ENTITY_EXIST(eParam0))
		DECORATOR::DECOR_SET_BOOL(eParam0, "chest_open", true);

	if (NETWORK::NETWORK_IS_IN_SESSION())
		func_57(func_56(joaat("chests_looted")), 1);
	else
		func_57(func_56(joaat("chests_searched")), 1);

	return;
}

void func_32(var uParam0) // Position - 0xEF2 Signature - 22 01 03 00 00 50 01 00 22 01 03 00 00 66 00 27
{
	return;
}

void func_33(var uParam0) // Position - 0xEFA Signature - 22 01 03 00 00 66 00 27 01
{
	if (uParam0->f_1 != pedLocal_21)
		return;

	if (TASK::_GET_SCENARIO_CONTAINER_OPENING_STATE(pedLocal_21.f_5))
		if (!func_58(pedLocal_21.f_5))
			func_31(pedLocal_21.f_5);

	return;
}

void func_34(var uParam0) // Position - 0xF30 Signature - 22 01 03 00 00 50 01 00 22 01 03 00 00 66 00 76
{
	return;
}

BOOL func_35(var uParam0) // Position - 0xF38 Signature - 22 01 03 00 00 66 00 76 25
{
	return !(DECORATOR::DECOR_EXIST_ON(*uParam0, "chest_local_only") && DECORATOR::DECOR_GET_BOOL(*uParam0, "chest_local_only"));
}

void func_36(var uParam0) // Position - 0xF5C Signature - 22 01 03 00 00 66 00 27 07
{
	switch (uParam0->f_7)
	{
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENLOCKED_TIGHT_0m0"):
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENLOCKED_0m80"):
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENUNLOCKED_0m60"):
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENLOCKED_TIGHT_0m45"):
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENUNLOCKED_0m45"):
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENLOCKED_0m60"):
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENLOCKED_0m45"):
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENUNLOCKED_0m80"):
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENUNLOCKED_0m0"):
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENLOCKED_0m0"):
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENLOCKED_TIGHT_0m60"):
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENLOCKED_TIGHT_0m80"):
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENLOCKED_1m05"):
		case joaat("RANSACK_PROP_LOCKBOX_MED_OPENUNLOCKED_1m05"):
			uParam0->f_8 = 14;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_PROP_DRAWER_SINGLE_4_OPEN"):
			uParam0->f_8 = 16;
			func_60(uParam0, func_59(uParam0, 3));
			uParam0->f_4 = 1;
			break;
	
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENUNLOCKED_0m45"):
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENLOCKED_1m05"):
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENUNLOCKED_0m60"):
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENUNLOCKED_0m80"):
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENLOCKED_TIGHT_0m60"):
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENLOCKED_TIGHT_0m80"):
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENLOCKED_0m80"):
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENLOCKED_0m0"):
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENUNLOCKED_0m0"):
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENLOCKED_TIGHT_0m45"):
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENLOCKED_0m45"):
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENUNLOCKED_1m05"):
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENLOCKED_TIGHT_0m0"):
		case joaat("RANSACK_PROP_LOCKBOX_SML_OPENLOCKED_0m60"):
			uParam0->f_8 = 15;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_PROP_DRAWER_DOUBLE_4_OPEN"):
			uParam0->f_8 = 17;
			func_60(uParam0, func_59(uParam0, 7));
			uParam0->f_4 = 1;
			break;
	
		case joaat("RANSACK_PROP_CHEST_LARGE_OPEN"):
			uParam0->f_8 = 1;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_PROP_CUPBOARD_NARROW_MID_OPEN"):
			uParam0->f_8 = 5;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_ATTACHED_DD_123"):
		case joaat("ransack_attached_dd_234"):
		case joaat("RANSACK_ATTACHED_DD_1234"):
		case joaat("RANSACK_ATTACHED_DD_34"):
			uParam0->f_8 = 17;
			func_60(uParam0, func_59(uParam0, 4));
			uParam0->f_4 = 1;
			break;
	
		case joaat("RANSACK_ATTACHED_DS_12"):
		case joaat("RANSACK_ATTACHED_DS_234"):
		case joaat("RANSACK_ATTACHED_DS_23"):
		case joaat("RANSACK_ATTACHED_DS_123"):
			uParam0->f_8 = 16;
			func_60(uParam0, func_59(uParam0, 0));
			uParam0->f_4 = 1;
			break;
	
		case joaat("RANSACK_PROP_DRAWER_SINGLE_3_OPEN"):
			uParam0->f_8 = 16;
			func_60(uParam0, func_59(uParam0, 2));
			uParam0->f_4 = 1;
			break;
	
		case joaat("RANSACK_PROP_CUPBOARD_NARROW_LOW_OPEN_B"):
		case joaat("RANSACK_PROP_CUPBOARD_NARROW_LOW_OPEN_A"):
			uParam0->f_8 = 6;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_PROP_DRAWER_SINGLE_2_OPEN"):
			uParam0->f_8 = 16;
			func_60(uParam0, func_59(uParam0, 1));
			uParam0->f_4 = 1;
			break;
	
		case joaat("RANSACK_PROP_DRAWER_DOUBLE_2_OPEN"):
			uParam0->f_8 = 17;
			func_60(uParam0, func_59(uParam0, 5));
			uParam0->f_4 = 1;
			break;
	
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENLOCKED_TIGHT_0m60"):
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENUNLOCKED_1m05"):
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENLOCKED_TIGHT_0m45"):
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENUNLOCKED_0m60"):
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENLOCKED_TIGHT_0m80"):
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENLOCKED_TIGHT_0m0"):
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENLOCKED_0m80"):
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENUNLOCKED_0m0"):
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENLOCKED_0m45"):
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENLOCKED_1m05"):
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENUNLOCKED_0m45"):
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENLOCKED_0m60"):
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENUNLOCKED_0m80"):
		case joaat("RANSACK_PROP_LOCKBOX_LRG_OPENLOCKED_0m0"):
			uParam0->f_8 = 13;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_PROP_CUPBOARD_WIDE_HIGH_OPEN"):
			uParam0->f_8 = 4;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_PROP_CHEST_MEDIUM_OPEN"):
			uParam0->f_8 = 0;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_PROP_CUPBOARD_WIDE_LOW_OPEN"):
			uParam0->f_8 = 10;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_PROP_DRAWER_DOUBLE_3_OPEN"):
			uParam0->f_8 = 17;
			func_60(uParam0, func_59(uParam0, 6));
			uParam0->f_4 = 1;
			break;
	
		case joaat("RANSACK_PROP_CUPBOARD_WIDE_LOW_RIGHT_OPEN"):
			uParam0->f_8 = 12;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_PROP_CUPBOARD_NARROW_LOW_RIGHT_OPEN"):
			uParam0->f_8 = 8;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_PROP_CUPBOARD_WIDE_MID_OPEN"):
			uParam0->f_8 = 9;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_PROP_CUPBOARD_NARROW_HIGH_OPEN_B"):
		case joaat("RANSACK_PROP_CUPBOARD_NARROW_HIGH_OPEN_A"):
			if (ENTITY::GET_ENTITY_MODEL(uParam0->f_5) == joaat("p_medicinechest01x"))
				uParam0->f_8 = 2;
			else
				uParam0->f_8 = 3;
		
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_PROP_DRAWER_DOUBLE_1_OPEN"):
			uParam0->f_8 = 17;
			func_60(uParam0, func_59(uParam0, 4));
			uParam0->f_4 = 1;
			break;
	
		case joaat("RANSACK_PROP_CUPBOARD_WIDE_LOW_LEFT_OPEN"):
			uParam0->f_8 = 11;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		case joaat("RANSACK_PROP_DRAWER_SINGLE_1_OPEN"):
			uParam0->f_8 = 16;
			func_60(uParam0, func_59(uParam0, 0));
			uParam0->f_4 = 1;
			break;
	
		case joaat("RANSACK_PROP_CUPBOARD_NARROW_LOW_LEFT_OPEN"):
			uParam0->f_8 = 7;
			func_60(uParam0, func_59(uParam0, -1));
			break;
	
		default:
			uParam0->f_8 = -1;
			break;
	}

	return;
}

void func_37(var uParam0) // Position - 0x138F Signature - 22 01 03 00 00 66 00 27 08
{
	switch (uParam0->f_8)
	{
		case 0:
			uParam0->f_99[0] = 10;
			break;
	
		case 1:
			uParam0->f_99[0] = 10;
			break;
	
		case 2:
			uParam0->f_99[0] = 150;
			break;
	
		case 3:
			uParam0->f_99[0] = 105;
			uParam0->f_99[1] = 150;
			break;
	
		case 4:
			uParam0->f_99[0] = 105;
			uParam0->f_99[1] = 150;
			break;
	
		case 5:
			uParam0->f_99[0] = 105;
			uParam0->f_99[1] = 150;
			break;
	
		case 6:
			uParam0->f_99[0] = 10;
			break;
	
		case 7:
			uParam0->f_99[0] = 10;
			break;
	
		case 8:
			uParam0->f_99[0] = 10;
			break;
	
		case 9:
			uParam0->f_99[0] = 80;
			uParam0->f_99[1] = 105;
			uParam0->f_99[2] = 150;
			break;
	
		case 10:
			uParam0->f_99[0] = 10;
			break;
	
		case 11:
			uParam0->f_99[0] = 10;
			break;
	
		case 12:
			uParam0->f_99[0] = 10;
			break;
	
		case 13:
			uParam0->f_99[0] = 5;
			break;
	
		case 14:
			uParam0->f_99[0] = 5;
			break;
	
		case 15:
			uParam0->f_99[0] = 5;
			break;
	
		case 16:
			break;
	
		case 17:
			break;
	}

	return;
}

void func_38(var uParam0) // Position - 0x14FD Signature - 22 01 1B
{
	int i;
	int endRange;
	float num;
	float num2;
	float num3;
	float num4;
	float num5;
	float num6;
	float num7;
	float num8;
	int num9;
	int num10;
	int num11;
	var unk;

	num9 = 1;
	num10 = 1;
	num11 = 1;
	unk = 10;

	switch (uParam0->f_8)
	{
		case 0:
			func_65(uParam0);
			return;
	
		case 1:
			num = -0.55f;
			num2 = 0.55f;
			num3 = -0.2f;
			num4 = 0.2f;
			break;
	
		case 2:
			num = 0.14f;
			num2 = 0.56f;
			num3 = 0.05f;
			num4 = 0.16f;
			num8 = 1.6f;
			break;
	
		case 3:
			num = 0.07f;
			num2 = 0.63f;
			num3 = 0.05f;
			num4 = 0.4f;
			num8 = 1.6f;
			break;
	
		case 4:
			num = 0.07f;
			num2 = 1.13f;
			num3 = 0.05f;
			num4 = 0.4f;
			num8 = 1.6f;
			break;
	
		case 5:
			func_61(uParam0);
			return;
	
		case 6:
			func_62(uParam0);
			return;
	
		case 7:
			num = 0.12f;
			num2 = 0.21f;
			num3 = 0.05f;
			num4 = 0.25f;
			num8 = 0.45f;
			break;
	
		case 8:
			num = -1f * 0.21f;
			num2 = -1f * 0.12f;
			num3 = 0.05f;
			num4 = 0.25f;
			num8 = 0.45f;
			break;
	
		case 9:
			num = 0.07f;
			num2 = 1.13f;
			num3 = 0.05f;
			num4 = 0.4f;
			num8 = 1.3f;
			break;
	
		case 10:
			num = 0.07f;
			num2 = 1.13f;
			num3 = 0.05f;
			num4 = 0.25f;
			num8 = 0.45f;
			break;
	
		case 11:
			num = 0.12f;
			num2 = 0.48f;
			num3 = 0.05f;
			num4 = 0.25f;
			num8 = 0.45f;
			break;
	
		case 12:
			num = -1f * 0.48f;
			num2 = -1f * 0.12f;
			num3 = 0.05f;
			num4 = 0.25f;
			num8 = 0.45f;
			break;
	
		case 13:
			func_66(uParam0);
			return;
	
		case 14:
			func_67(uParam0);
			return;
	
		case 15:
			func_68(uParam0);
			return;
	
		case 16:
			func_63(uParam0);
			return;
	
		case 17:
			func_64(uParam0);
			return;
	}

	num6 = num3;
	num4 = num4;

	for (i = 0; i < 3; i = i + 1)
	{
		if (uParam0->f_99[i] != -1)
			endRange = endRange + 1;
	}

	func_69(&unk, 10, true);

	for (i = 0; i < 10; i = i + 1)
	{
		num5 = func_70(num5, num, num2, 0.15f, 0.25f, &num10);
	
		if (num9 != num10)
		{
			num6 = func_70(num6, num3, num4, 0.13f, 0.13f, &num11);
			num9 = num10;
		}
	
		if (endRange > 0)
			num7 = (BUILTIN::TO_FLOAT(uParam0->f_99[MISC::GET_RANDOM_INT_IN_RANGE(0, endRange)]) / 100f) - num8;
	
		uParam0->f_37[unk[i] /*3*/] = { num5, num6, num7 };
	}

	return;
}

BOOL func_39(var uParam0) // Position - 0x1820 Signature - 22 01 09
{
	Entity entityAttachedTo;
	PropSet vehiclePropSet;
	ItemSet itemSet;
	int entitiesFromPropSet;
	int i;
	ScrHandle indexedItemInItemset;

	if (!ENTITY::IS_ENTITY_ATTACHED_TO_ANY_VEHICLE(uParam0->f_5))
		return false;

	entityAttachedTo = ENTITY::GET_ENTITY_ATTACHED_TO(uParam0->f_5);

	if (VEHICLE::IS_THIS_MODEL_A_TRAIN(ENTITY::GET_ENTITY_MODEL(entityAttachedTo)))
		return false;

	if (!PROPSET::_DOES_VEHICLE_HAVE_ANY_PROP_SET(ENTITY::GET_VEHICLE_INDEX_FROM_ENTITY_INDEX(entityAttachedTo)))
		return false;

	vehiclePropSet = PROPSET::_GET_VEHICLE_PROP_SET(ENTITY::GET_VEHICLE_INDEX_FROM_ENTITY_INDEX(entityAttachedTo));
	itemSet = ITEMSET::CREATE_ITEMSET(true);
	entitiesFromPropSet = PROPSET::_GET_ENTITIES_FROM_PROP_SET(vehiclePropSet, itemSet, ENTITY::GET_ENTITY_MODEL(uParam0->f_5), false, false);

	if (entitiesFromPropSet <= 0)
	{
		ITEMSET::DESTROY_ITEMSET(itemSet);
		return false;
	}

	for (i = 0; i < entitiesFromPropSet; i = i + 1)
	{
		indexedItemInItemset = ITEMSET::GET_INDEXED_ITEM_IN_ITEMSET(i, itemSet);
	
		if (uParam0->f_5 == MISC::_GET_ENTITY_FROM_ITEM(indexedItemInItemset))
		{
			ITEMSET::DESTROY_ITEMSET(itemSet);
			return true;
		}
	}

	ITEMSET::DESTROY_ITEMSET(itemSet);
	return false;
}

BOOL func_40(Vector3 vParam0, var uParam1, var uParam2) // Position - 0x18D7 Signature - 22 03 06
{
	Interior interiorFromCollision;

	if (!INTERIOR::IS_COLLISION_MARKED_OUTSIDE(vParam0))
	{
		interiorFromCollision = INTERIOR::GET_INTERIOR_FROM_COLLISION(vParam0);
	
		if (INTERIOR::IS_VALID_INTERIOR(interiorFromCollision))
			return true;
	}

	return false;
}

Object func_41(Hash hParam0, Vector3 vParam1, var uParam2, var uParam3, int iParam4, Hash hParam5) // Position - 0x1903 Signature - 22 06 0A 00 00 66 00
{
	Object objectIndexFromEntityIndex;
	Entity propForScenarioPoint;

	STREAMING::REQUEST_MODEL(hParam0, false);

	if (!STREAMING::HAS_MODEL_LOADED(hParam0))
		return 0;

	if (!TASK::DOES_SCENARIO_POINT_EXIST(iParam4) && hParam5 != 0)
		iParam4 = TASK::FIND_SCENARIO_OF_TYPE_HASH(vParam1, hParam5, 1f, 0, false);

	if (TASK::DOES_SCENARIO_POINT_EXIST(iParam4))
	{
		if (TASK::_DOES_SCENARIO_POINT_HAVE_PROPS(iParam4))
		{
			propForScenarioPoint = TASK::GET_PROP_FOR_SCENARIO_POINT(iParam4, "CONTAINER");
		
			if (ENTITY::IS_ENTITY_AN_OBJECT(propForScenarioPoint))
				objectIndexFromEntityIndex = ENTITY::GET_OBJECT_INDEX_FROM_ENTITY_INDEX(propForScenarioPoint);
		}
	}
	else
	{
		objectIndexFromEntityIndex = OBJECT::GET_CLOSEST_OBJECT_OF_TYPE(vParam1, 0.5f, hParam0, false, false, true);
	}

	return objectIndexFromEntityIndex;
}

void func_42(Entity eParam0) // Position - 0x198D Signature - 22 01 03 00 00 66 00 03 05 00 54 8B 18
{
	if (ENTITY::DOES_ENTITY_EXIST(eParam0))
	{
		DECORATOR::DECOR_SET_BOOL(eParam0, "scripted_loot_only", true);
		ENTITY::SCRIPT_OVERRIDE_ENTITY_LOOT_TABLE_PERMANENT(eParam0, joaat("empty"));
	}

	return;
}

int func_43() // Position - 0x19B6 Signature - 22 00 02 00 00 37
{
	return joaat("p_dressinglcmhay01x");
}

int func_44() // Position - 0x19C3 Signature - 22 00 04
{
	int randomIntInRange;
	int num;

	randomIntInRange = MISC::GET_RANDOM_INT_IN_RANGE(0, 4);
	num = 0;

	switch (randomIntInRange)
	{
		case 0:
			num = joaat("s_lootablebedchest_a");
			break;
	
		case 1:
			num = joaat("s_lootablebedchest_b");
			break;
	
		case 2:
			num = joaat("s_lootablebedchest_c");
			break;
	
		case 3:
			num = joaat("s_lootablebedchest_d");
			break;
	}

	return 0;
}

int func_45() // Position - 0x1A1F Signature - 22 00 03 00 00 2F 67 02 66 02 50
{
	int num;

	num = 0;
	return num;
}

int func_46(var uParam0) // Position - 0x1A2C Signature - 22 01 04 00 00 2F 67 03 66 03 50
{
	int num;

	num = 0;
	return num;
}

int func_47(Entity eParam0) // Position - 0x1A39 Signature - 22 01 03 00 00 66 00 03 05 00 54 05
{
	if (!ENTITY::DOES_ENTITY_EXIST(eParam0))
		return 0;

	if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_empty"))
		return 1;

	return 0;
}

BOOL func_48(Entity eParam0) // Position - 0x1A60 Signature - 22 01 03 00 00 66 00 03 05 00 54 6A
{
	if (ENTITY::DOES_ENTITY_EXIST(eParam0) && DECORATOR::DECOR_EXIST_ON(eParam0, "scripted_loot_only") && DECORATOR::DECOR_GET_BOOL(eParam0, "scripted_loot_only"))
		return true;

	return false;
}

void func_49(var uParam0, var uParam1, int iParam2) // Position - 0x1A94 Signature - 22 03 12
{
	Hash hash;
	var unk;
	var unk2;
	var unk5;
	int num;
	var unk8;
	var unk9;
	var unk10;
	var unk11;

	while (!func_20(uParam0->f_5) && num < iParam2)
	{
		hash = func_71(uParam0->f_5, &unk8, &unk, &unk10, &unk11, &unk2, &unk5, &unk9);
	
		if (func_72(hash, 0))
		{
			func_73(uParam0, unk, unk2, unk5, hash, unk8, unk10, unk11, unk9);
			uParam1->f_1[num] = hash;
			num = num + 1;
		}
	}

	return;
}

void func_50(var uParam0, float fParam1) // Position - 0x1B01 Signature - 22 02 04 00 00 39
{
	uParam0->f_1 = func_74() - fParam1;
	func_75(uParam0, 1);
	func_76(uParam0, 2);
	uParam0->f_2 = 0f;
	return;
}

BOOL func_51(var uParam0) // Position - 0x1B27 Signature - 22 01 03 00 00 66 00 76 09
{
	return func_77(*uParam0, 1);
}

float func_52(var uParam0) // Position - 0x1B37 Signature - 22 01 03 00 00 66 00 39 27
{
	if (!func_51(uParam0))
		return uParam0->f_1;

	if (func_78(uParam0))
		return uParam0->f_2;

	return func_74() - uParam0->f_1;
}

void func_53(var uParam0, var uParam1, Hash hParam2, int iParam3) // Position - 0x1B6C Signature - 22 04
{
	if (hParam2 == 0)
		hParam2 = ENTITY::GET_ENTITY_MODEL(*uParam1);

	if (hParam2 == joaat("mp004_s_mp_bondrolled01x") || iParam3 == joaat("kit_placed_explosives"))
		DECORATOR::DECOR_SET_BOOL(*uParam1, "loot_mp_mission_only", true);

	return;
}

var func_54(BOOL bParam0, var uParam1, var uParam2) // Position - 0x1BA8 Signature - 22 03 05 00 00 66 00 8B 08 00 66 01 50 03 01 68 05 00 66 02 50 03 01 22 07
{
	if (bParam0)
		return uParam1;

	return uParam2;
}

Vector3 func_55(BOOL bParam0, var uParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6) // Position - 0x1BBF Signature - 22 07 09 00 00 66 00
{
	if (bParam0)
		return uParam1;

	return uParam4;
}

struct<2> func_56(int iParam0) // Position - 0x1BDA Signature - 22 01 05 00 00 66
{
	var unk;

	unk.f_1 = iParam0;
	return unk;
}

void func_57(var uParam0, var uParam1, int iParam2) // Position - 0x1BEC Signature - 22 03 05 00 00 4B
{
	if (!STATS::STAT_ID_IS_VALID(&uParam0))
		return;

	STATS::_STAT_ID_INCREMENT_INT(&uParam0, iParam2);
	return;
}

BOOL func_58(Entity eParam0) // Position - 0x1C09 Signature - 22 01 03 00 00 66 00 03 05 00 54 8B 0D
{
	if (ENTITY::DOES_ENTITY_EXIST(eParam0))
		return DECORATOR::DECOR_EXIST_ON(eParam0, "chest_open");

	return false;
}

int func_59(var uParam0, int iParam1) // Position - 0x1C28 Signature - 22 02 05 00 00 66 00 66
{
	int entityBoneIndexByName;

	if (!MISC::IS_STRING_NULL_OR_EMPTY(func_79(uParam0, iParam1)))
	{
		entityBoneIndexByName = ENTITY::GET_ENTITY_BONE_INDEX_BY_NAME(uParam0->f_5, func_79(uParam0, iParam1));
		return entityBoneIndexByName;
	}

	return 0;
}

void func_60(var uParam0, int iParam1) // Position - 0x1C58 Signature - 22 02 05 00 00 2F
{
	int i;

	for (i = 0; i < 10; i = i + 1)
	{
		uParam0->f_154[i] = iParam1;
	}

	return;
}

void func_61(var uParam0) // Position - 0x1C7D Signature - 22 01 2E 00 00 4B 03 6D 0A 3D 41 4B 22 6D 0A 3D 41 86 CD
{
	var unk;
	var unk32;
	int i;

	unk = 10;
	unk32 = 10;
	unk[0 /*3*/] = { 0.05f, 0.07f, 0.2f };
	unk[1 /*3*/] = { 0.17f, 0.21f, 0.2f };
	unk[2 /*3*/] = { 0.28f, 0.07f, 0.2f };
	unk[3 /*3*/] = { 0.39f, 0.23f, 0.2f };
	unk[4 /*3*/] = { 0.41f, 0.08f, 0.2f };
	unk[5 /*3*/] = { 0.08f, 0.08f, -0.25f };
	unk[6 /*3*/] = { 0.22f, 0.19f, -0.25f };
	unk[7 /*3*/] = { 0.35f, 0.09f, -0.25f };
	unk[8 /*3*/] = { 0.49f, 0.19f, -0.25f };
	unk[9 /*3*/] = { 0.63f, 0.08f, -0.25f };
	func_69(&unk32, 10, true);

	for (i = 0; i < 9; i = i + 1)
	{
		uParam0->f_37[i /*3*/] = { unk[unk32[i] /*3*/] };
	}

	return;
}

void func_62(var uParam0) // Position - 0x1DA1 Signature - 22 01 2E 00 00 4B 03 6D 0A 3D 41 4B 22 6D 0A 3D 41 86 0A D7 A3
{
	var unk;
	var unk32;
	int i;

	unk = 10;
	unk32 = 10;
	unk[0 /*3*/] = { 0.08f, 0.08f, 0.15f };
	unk[1 /*3*/] = { 0.22f, 0.19f, 0.15f };
	unk[2 /*3*/] = { 0.35f, 0.09f, 0.15f };
	unk[3 /*3*/] = { 0.49f, 0.19f, 0.15f };
	unk[4 /*3*/] = { 0.63f, 0.08f, 0.15f };
	unk[5 /*3*/] = { 0.08f, 0.08f, -0.35f };
	unk[6 /*3*/] = { 0.22f, 0.19f, -0.35f };
	unk[7 /*3*/] = { 0.35f, 0.09f, -0.35f };
	unk[8 /*3*/] = { 0.49f, 0.19f, -0.35f };
	unk[9 /*3*/] = { 0.63f, 0.08f, -0.35f };
	func_69(&unk32, 10, true);

	for (i = 0; i < 9; i = i + 1)
	{
		uParam0->f_37[i /*3*/] = { unk[unk32[i] /*3*/] };
	}

	return;
}

void func_63(var uParam0) // Position - 0x1EC5 Signature - 22 01 2E 00 00 4B 03 6D 0A 3D 41 4B 22 6D 0A 3D 41 73 86 EC
{
	var unk;
	var unk32;
	int i;

	unk = 10;
	unk32 = 10;
	unk[0 /*3*/] = { 0f, 0.18f, -0.03f };
	unk[1 /*3*/] = { -0.09f, 0.2f, -0.03f };
	unk[2 /*3*/] = { 0f, 0.27f, -0.03f };
	unk[3 /*3*/] = { 0.09f, 0.16f, -0.03f };
	unk[4 /*3*/] = { 0f, 0.09f, -0.03f };
	unk[5 /*3*/] = { -0.12f, 0.31f, -0.03f };
	unk[6 /*3*/] = { 0.12f, 0.13f, -0.03f };
	unk[7 /*3*/] = { 0.12f, 0.13f, -0.03f };
	unk[8 /*3*/] = { -0.12f, 0.31f, -0.03f };
	unk[9 /*3*/] = { -0.08f, 0.15f, -0.03f };

	for (i = 0; i < 9; i = i + 1)
	{
		uParam0->f_37[i /*3*/] = { unk[unk32[i] /*3*/] };
	}

	return;
}

void func_64(var uParam0) // Position - 0x1FD4 Signature - 22 01 2E 00 00 4B 03 6D 0A 3D 41 4B 22 6D 0A 3D 41 86 0A D7 23
{
	var unk;
	var unk32;
	int i;

	unk = 10;
	unk32 = 10;
	unk[0 /*3*/] = { 0.04f, 0.17f, -0.04f };
	unk[1 /*3*/] = { -0.13f, 0.22f, -0.04f };
	unk[2 /*3*/] = { -0.26f, 0.13f, -0.04f };
	unk[3 /*3*/] = { -0.4f, 0.23f, -0.04f };
	unk[4 /*3*/] = { -0.57f, 0.18f, -0.04f };
	unk[5 /*3*/] = { -0.56f, 0.33f, -0.04f };
	unk[6 /*3*/] = { -0.41f, 0.37f, -0.04f };
	unk[7 /*3*/] = { -0.23f, 0.36f, -0.04f };
	unk[8 /*3*/] = { -0.05f, 0.38f, -0.04f };
	unk[9 /*3*/] = { 0.08f, 0.34f, -0.04f };
	func_69(&unk32, 10, true);

	for (i = 0; i < 9; i = i + 1)
	{
		uParam0->f_37[i /*3*/] = { unk[unk32[i] /*3*/] };
	}

	return;
}

void func_65(var uParam0) // Position - 0x20F8 Signature - 22 01 2E 00 00 4B 03 6D 0A 3D 41 4B 22 6D 0A 3D 41 86 7B
{
	var unk;
	var unk32;
	int i;

	unk = 10;
	unk32 = 10;
	unk[0 /*3*/] = { -0.34f, -0.11f, 0.11f };
	unk[1 /*3*/] = { -0.32f, -0.02f, 0.11f };
	unk[2 /*3*/] = { -0.19f, -0.14f, 0.11f };
	unk[3 /*3*/] = { -0.1f, -0.01f, 0.11f };
	unk[4 /*3*/] = { -0.32f, 0.17f, 0.11f };
	unk[5 /*3*/] = { -0.15f, 0.18f, 0.11f };
	unk[6 /*3*/] = { 0.06f, 0.13f, 0.11f };
	unk[7 /*3*/] = { 0.13f, 0.03f, 0.11f };
	unk[8 /*3*/] = { 0.27f, 0.12f, 0.11f };
	unk[9 /*3*/] = { 0.28f, -0.03f, 0.11f };
	func_69(&unk32, 10, true);

	for (i = 0; i < 10; i = i + 1)
	{
		uParam0->f_68[i /*3*/] = { 0f, 0f, 90f };
		uParam0->f_37[i /*3*/] = { unk[unk32[i] /*3*/] };
	}

	return;
}

void func_66(var uParam0) // Position - 0x222D Signature - 22 01 2E 00 00 4B 03 6D 0A 3D 41 4B 22 6D 0A 3D 41 73 86 29
{
	var unk;
	var unk32;
	int i;

	unk = 10;
	unk32 = 10;
	unk[0 /*3*/] = { 0f, 0.07f, 0.02f };
	unk[1 /*3*/] = { 0f, -0.07f, 0.02f };
	unk[2 /*3*/] = { -0.16f, -0.07f, 0.02f };
	unk[3 /*3*/] = { 0.16f, -0.07f, 0.02f };
	unk[4 /*3*/] = { -0.16f, 0.07f, 0.02f };
	unk[5 /*3*/] = { 0.16f, 0.07f, 0.02f };
	uParam0->f_2 = 6;
	func_69(&unk32, uParam0->f_2, true);

	for (i = 0; i < uParam0->f_2; i = i + 1)
	{
		uParam0->f_68[i /*3*/] = { 0f, 0f, 90f };
		uParam0->f_37[i /*3*/] = { unk[unk32[i] /*3*/] };
	}

	return;
}

void func_67(var uParam0) // Position - 0x2309 Signature - 22 01 2E 00 00 4B 03 6D 0A 3D 41 4B 22 6D 0A 3D 41 86 AE
{
	var unk;
	var unk32;
	int i;

	unk = 10;
	unk32 = 10;
	unk[0 /*3*/] = { -0.11f, 0.07f, 0.02f };
	unk[1 /*3*/] = { 0.11f, 0.07f, 0.02f };
	unk[2 /*3*/] = { -0.11f, -0.07f, 0.02f };
	unk[3 /*3*/] = { 0.11f, -0.07f, 0.02f };
	unk[4 /*3*/] = { 0f, 0f, 0.02f };
	uParam0->f_2 = 5;
	func_69(&unk32, uParam0->f_2, true);

	for (i = 0; i < uParam0->f_2; i = i + 1)
	{
		uParam0->f_68[i /*3*/] = { 0f, 0f, 90f };
		uParam0->f_37[i /*3*/] = { unk[unk32[i] /*3*/] };
	}

	return;
}

void func_68(var uParam0) // Position - 0x23CF Signature - 22 01 2F
{
	var unk;
	var unk32;
	float num;
	int i;

	unk = 10;
	unk32 = 10;
	num = 0.005f;

	for (i = 0; i < 9; i = i + 1)
	{
		unk[i /*3*/] = { 0f, 0f, num };
		num = num + 0.02f;
		uParam0->f_68[i /*3*/] = { 0f, 0f, 90f };
	}

	uParam0->f_2 = 2;
	func_69(&unk32, uParam0->f_2, true);

	for (i = 0; i < uParam0->f_2; i = i + 1)
	{
		uParam0->f_37[i /*3*/] = { unk[unk32[i] /*3*/] };
	}

	return;
}

void func_69(var uParam0, int iParam1, BOOL bParam2) // Position - 0x245F Signature - 22 03 08
{
	var unk;
	int randomIntInRange;
	int i;

	i = 0;

	if (bParam2)
	{
		for (i = 0; i <= iParam1 - 1; i = i + 1)
		{
			uParam0->[i] = i;
		}
	}

	i = iParam1 - 1;

	for (i = iParam1 - 1; i >= 0; i = i + -1)
	{
		randomIntInRange = MISC::GET_RANDOM_INT_IN_RANGE(0, i);
		unk = uParam0->[i];
		uParam0->[i] = uParam0->[randomIntInRange];
		uParam0->[randomIntInRange] = unk;
	}

	return;
}

float func_70(float fParam0, float fParam1, float fParam2, float fParam3, float fParam4, var uParam5) // Position - 0x24CC Signature - 22 06 0A 00 00 66 01
{
	float num;
	float num2;

	if (fParam1 == fParam2)
		return fParam1;

	num = func_80() ? fParam3 : fParam4;

	if (*uParam5)
	{
		if (num + fParam0 > fParam2)
		{
			*uParam5 = 0;
			num2 = fParam2 - MISC::ABSF((num + fParam0) - fParam2);
		}
		else
		{
			num2 = num + fParam0;
		}
	}
	else if (fParam0 - num < fParam1)
	{
		*uParam5 = 1;
		num2 = fParam1 + MISC::ABSF(fParam0 - num - fParam1);
	}
	else
	{
		num2 = fParam0 - num;
	}

	return num2;
}

Hash func_71(Entity eParam0, var uParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6, var uParam7) // Position - 0x254E Signature - 22 08
{
	Hash ammoTypeForWeapon;
	Hash _int;
	float _float;
	float _float2;
	float _float3;
	int i;
	var unk;
	var propertyName;
	var propertyName2;
	Hash _int2;
	Hash _int3;

	ammoTypeForWeapon = 0;
	_int = 0;
	*uParam1 = 0;
	*uParam7 = -1;

	if (!ENTITY::DOES_ENTITY_EXIST(eParam0))
		return 0;

	if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_empty"))
		return 0;

	*uParam1 = 1;

	if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_money"))
	{
		*uParam1 = DECORATOR::DECOR_GET_INT(eParam0, "loot_money");
		DECORATOR::DECOR_REMOVE(eParam0, "loot_money");
		*uParam2 = 0;
		return joaat("currency_cash");
	}

	if (DECORATOR::DECOR_EXIST_ON(eParam0, "fLoot_money"))
	{
		*uParam3 = DECORATOR::DECOR_GET_FLOAT(eParam0, "fLoot_money");
		DECORATOR::DECOR_REMOVE(eParam0, "fLoot_money");
		*uParam2 = 0;
		return func_82(*uParam3);
	}

	if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_capitale"))
	{
		DECORATOR::DECOR_REMOVE(eParam0, "loot_capitale");
		*uParam2 = 4;
	
		if (Global_1901671.f_171.f_71)
			return 0;
		else
			return 757274294;
	}

	TEXT_LABEL_ASSIGN_STRING(&unk, "loot_ammo", 16);

	for (i = 1; i <= 3; i = i + 1)
	{
		propertyName = { unk };
		propertyName2 = { unk };
		TEXT_LABEL_APPEND_INT(&propertyName, i, 16);
		TEXT_LABEL_APPEND_INT(&propertyName2, i, 16);
		TEXT_LABEL_APPEND_STRING(&propertyName, "_type", 16);
		TEXT_LABEL_APPEND_STRING(&propertyName2, "_amt", 16);
	
		if (DECORATOR::DECOR_EXIST_ON(eParam0, &propertyName))
		{
			_int = DECORATOR::DECOR_GET_INT(eParam0, &propertyName);
		
			if (_int != 0)
			{
				ammoTypeForWeapon = WEAPON::_GET_AMMO_TYPE_FOR_WEAPON(_int);
				DECORATOR::DECOR_REMOVE(eParam0, &propertyName);
			
				if (DECORATOR::DECOR_EXIST_ON(eParam0, &propertyName2))
				{
					*uParam1 = DECORATOR::DECOR_GET_INT(eParam0, &propertyName2);
					DECORATOR::DECOR_REMOVE(eParam0, &propertyName2);
					*uParam2 = 2;
				}
			
				return ammoTypeForWeapon;
			}
		}
	}

	if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_weapon"))
	{
		_int = DECORATOR::DECOR_GET_INT(eParam0, "loot_weapon");
	
		if (_int != 0)
		{
			DECORATOR::DECOR_REMOVE(eParam0, "loot_weapon");
			*uParam2 = 3;
			return func_83(_int);
		}
	}

	if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_type1"))
	{
		if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_amt1"))
		{
			*uParam1 = DECORATOR::DECOR_GET_INT(eParam0, "loot_item_amt1");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_amt1");
		}
	
		if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_bone1"))
		{
			*uParam7 = DECORATOR::DECOR_GET_INT(eParam0, "loot_item_bone1");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_bone1");
		}
	
		if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_model1"))
		{
			*uParam4 = DECORATOR::DECOR_GET_INT(eParam0, "loot_item_model1");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_model1");
		}
	
		if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_offset1x") && DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_offset1y") && DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_offset1z"))
		{
			_float = DECORATOR::DECOR_GET_FLOAT(eParam0, "loot_item_offset1x");
			_float2 = DECORATOR::DECOR_GET_FLOAT(eParam0, "loot_item_offset1y");
			_float3 = DECORATOR::DECOR_GET_FLOAT(eParam0, "loot_item_offset1z");
			*uParam5 = { _float, _float2, _float3 };
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_offset1x");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_offset1y");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_offset1z");
		}
	
		if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_orient1x") && DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_orient1y") && DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_orient1z"))
		{
			_float = DECORATOR::DECOR_GET_FLOAT(eParam0, "loot_item_orient1x");
			_float2 = DECORATOR::DECOR_GET_FLOAT(eParam0, "loot_item_orient1y");
			_float3 = DECORATOR::DECOR_GET_FLOAT(eParam0, "loot_item_orient1z");
			*uParam6 = { _float, _float2, _float3 };
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_orient1x");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_orient1y");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_orient1z");
		}
	
		_int2 = DECORATOR::DECOR_GET_INT(eParam0, "loot_item_type1");
		DECORATOR::DECOR_REMOVE(eParam0, "loot_item_type1");
		*uParam2 = 1;
		return _int2;
	}

	if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_type2"))
	{
		if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_amt2"))
		{
			*uParam1 = DECORATOR::DECOR_GET_INT(eParam0, "loot_item_amt2");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_amt2");
		}
	
		if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_bone2"))
		{
			*uParam7 = DECORATOR::DECOR_GET_INT(eParam0, "loot_item_bone2");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_bone2");
		}
	
		if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_model2"))
		{
			*uParam4 = DECORATOR::DECOR_GET_INT(eParam0, "loot_item_model2");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_model2");
		}
	
		if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_offset2x") && DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_offset2y") && DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_offset2z"))
		{
			_float = DECORATOR::DECOR_GET_FLOAT(eParam0, "loot_item_offset2x");
			_float2 = DECORATOR::DECOR_GET_FLOAT(eParam0, "loot_item_offset2y");
			_float3 = DECORATOR::DECOR_GET_FLOAT(eParam0, "loot_item_offset2z");
			*uParam5 = { _float, _float2, _float3 };
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_offset2x");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_offset2y");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_offset2z");
		}
	
		if (DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_orient2x") && DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_orient2y") && DECORATOR::DECOR_EXIST_ON(eParam0, "loot_item_orient2z"))
		{
			_float = DECORATOR::DECOR_GET_FLOAT(eParam0, "loot_item_orient2x");
			_float2 = DECORATOR::DECOR_GET_FLOAT(eParam0, "loot_item_orient2y");
			_float3 = DECORATOR::DECOR_GET_FLOAT(eParam0, "loot_item_orient2z");
			*uParam6 = { _float, _float2, _float3 };
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_orient2x");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_orient2y");
			DECORATOR::DECOR_REMOVE(eParam0, "loot_item_orient2z");
		}
	
		_int3 = DECORATOR::DECOR_GET_INT(eParam0, "loot_item_type2");
		DECORATOR::DECOR_REMOVE(eParam0, "loot_item_type2");
		*uParam2 = 1;
		return _int3;
	}

	return 0;
}

BOOL func_72(Hash hParam0, int iParam1) // Position - 0x29FE Signature - 22 02 04 00 00 66 00 2F
{
	if (hParam0 == 0)
		return false;

	return ITEMDATABASE::_ITEMDATABASE_IS_KEY_VALID(hParam0, iParam1);
}

void func_73(var uParam0, var uParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6, var uParam7, Hash hParam8, var uParam9, var uParam10, var uParam11, var uParam12) // Position - 0x2A18 Signature - 22 0D
{
	int i;

	for (i = 0; i < 10; i = i + 1)
	{
		if (uParam0->f_103[i /*5*/] == -1)
		{
			uParam0->f_103[i /*5*/] = uParam1;
			uParam0->f_154[i] = uParam12;
			uParam0->f_103[i /*5*/].f_3 = hParam8;
			uParam0->f_103[i /*5*/].f_1 = uParam9;
			uParam0->f_103[i /*5*/].f_2 = uParam10;
			uParam0->f_103[i /*5*/].f_4 = uParam11;
		
			if (func_84(uParam0->f_103[i /*5*/].f_3) && func_85(uParam0->f_103[i /*5*/].f_3))
			{
				func_86(&uParam0->f_37[i /*3*/], &uParam0->f_68[i /*3*/]);
			}
			else
			{
				if (!_IS_NULL_VECTOR(uParam2))
					uParam0->f_37[i /*3*/] = { uParam2 };
			
				if (!_IS_NULL_VECTOR(uParam5))
					uParam0->f_68[i /*3*/] = { uParam5 };
			}
		
			break;
		}
	}

	return;
}

float func_74() // Position - 0x2AF5 Signature - 22 00 03 00 00 03
{
	int networkTimeAccurate;

	if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS())
	{
		networkTimeAccurate = NETWORK::GET_NETWORK_TIME_ACCURATE();
		return BUILTIN::TO_FLOAT(networkTimeAccurate) / 1000f;
	}

	return BUILTIN::TO_FLOAT(MISC::GET_GAME_TIMER()) / 1000f;
}

void func_75(var uParam0, int iParam1) // Position - 0x2B27 Signature - 22 02 04 00 00 66 00 76 66 01 30 66 00 32 50 02 00 22 02
{
	*uParam0 = *uParam0 || iParam1;
	return;
}

void func_76(var uParam0, int iParam1) // Position - 0x2B38 Signature - 22 02 04 00 00 66 00 76 66 00
{
	*uParam0 = *uParam0 - *uParam0 && iParam1;
	return;
}

BOOL func_77(int iParam0, int iParam1) // Position - 0x2B4D Signature - 22 02 04 00 00 66 00 66 01 69 2F 1C 50 02 01 22 01 03 00 00 66 00 76
{
	return iParam0 && iParam1 != false;
}

BOOL func_78(var uParam0) // Position - 0x2B5C Signature - 22 01 03 00 00 66 00 76 11
{
	return func_77(*uParam0, 2);
}

char* func_79(var uParam0, int iParam1) // Position - 0x2B6C Signature - 22 02 04 00 00 66 00 27
{
	switch (uParam0->f_8)
	{
		case -1:
			return "";
	
		case 0:
			return "Root_p_CHEST_MED";
	
		case 1:
			return "Root_p_CHEST_LRG";
	
		case 2:
		case 3:
			return "p_CUPBOARDS_NARROW_TalL";
	
		case 4:
			return "p_CUPBOARDS_WIDE_TalL";
	
		case 5:
			return "p_CUPBOARDS_NARROW_MedL";
	
		case 6:
			return "p_CUPBOARDS_NARROW_LowL";
	
		case 7:
			return "p_CUPBOARDS_NARROW_LowL";
	
		case 8:
			return "p_CUPBOARDS_NARROW_LowR";
	
		case 9:
			return "p_CUPBOARDS_WIDE_MedL";
	
		case 10:
			return "p_CUPBOARDS_WIDE_LowL";
	
		case 11:
			return "p_CUPBOARDS_WIDE_LowL";
	
		case 12:
			return "p_CUPBOARDS_WIDE_LowR";
	
		case 13:
			return "Root_p_LOCKBOX_LRG";
	
		case 14:
			return "Root_p_LOCKBOX_MED";
	
		case 15:
			return "Root_p_LOCKBOX_SML";
	
		case 16:
			return func_88(iParam1);
	
		case 17:
			return func_88(iParam1);
	}

	return "";
}

BOOL func_80() // Position - 0x2CAC Signature - 22 00 02 00 00 2F
{
	if (MISC::IS_BIT_SET(MISC::GET_RANDOM_INT_IN_RANGE(0, 65536), 0))
		return true;

	return false;
}

var func_81(BOOL bParam0, var uParam1, var uParam2) // Position - 0x2CCD Signature - 22 03 05 00 00 66 00 8B 08 00 66 01 50 03 01 68 05 00 66 02 50 03 01 22 01
{
	if (bParam0)
		return uParam1;

	return uParam2;
}

Hash func_82(var uParam0) // Position - 0x2CE4 Signature - 22 01 03 00 00 2F
{
	return 0;
}

Hash func_83(Hash hParam0) // Position - 0x2CED Signature - 22 01 03 00 00 66 00 03 05 00 2B
{
	if (WEAPON::IS_WEAPON_VALID(hParam0))
		return hParam0;

	return 0;
}

BOOL func_84(Hash hParam0) // Position - 0x2D04 Signature - 22 01 03 00 00 66 00 39 0E
{
	return func_89(hParam0) == joaat("WEAPON");
}

BOOL func_85(Hash hParam0) // Position - 0x2D18 Signature - 22 01 03 00 00 66 00 03 05 00 1B
{
	return WEAPON::IS_WEAPON_BOW(hParam0);
}

void func_86(var uParam0, var uParam1) // Position - 0x2D26 Signature - 22 02 04 00 00 86
{
	*uParam0 = { -0.12f, 0.12f, 0.12f };
	*uParam1 = { 90f, 0f, 115f };
	return;
}

BOOL _IS_NULL_VECTOR(float fParam0, var uParam1, var uParam2) // Position - 0x2D50 Signature - 22 03 05 00 00 66 00 73
{
	if (fParam0 == 0f && fParam0.f_1 == 0f && fParam0.f_2 == 0f)
		return true;

	return false;
}

char* func_88(int iParam0) // Position - 0x2D7A Signature - 22 01 03 00 00 66 00 3C
{
	switch (iParam0)
	{
		case 0:
			return "p_DRAWER_SGL_Drawer1";
	
		case 1:
			return "p_DRAWER_SGL_Drawer2";
	
		case 2:
			return "p_DRAWER_SGL_Drawer3";
	
		case 3:
			return "p_DRAWER_SGL_Drawer4";
	
		case 4:
			return "p_DRAWER_DBL_Drawer1_NUB";
	
		case 5:
			return "p_DRAWER_DBL_Drawer2_NUB";
	
		case 6:
			return "p_DRAWER_DBL_Drawer3_NUB";
	
		case 7:
			return "p_DRAWER_DBL_Drawer4_NUB";
	}

	return "";
}

int func_89(Hash hParam0) // Position - 0x2E0E Signature - 22 01 0A
{
	var unk;

	if (!func_72(hParam0, 0))
		return 0;

	if (!ITEMDATABASE::ITEMDATABASE_FILLOUT_ITEM_INFO(hParam0, &unk))
		return 0;

	return unk.f_2;
}

